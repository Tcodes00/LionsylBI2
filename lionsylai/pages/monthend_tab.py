...
def _checklist():
    ...
    if "me_checklist" not in st.session_state:
        st.session_state["me_checklist"] = {item: False for item in [
    ...
    for cat, items in categories.items():
        ...
        for item in items:
            ...
            with col1:
                checked = st.checkbox(item, value=chk[item], key=f"chk_{item[:20]}")
                chk[item] = checked
            with col2:
    ...
    if pct < 100:
        ...
        if st.button("🎉 Mark Month-End Close COMPLETE", type="primary",
            ...
            st.session_state["me_checklist"] = {k: False for k in chk}
            st.rerun()
